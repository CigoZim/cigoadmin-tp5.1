...
class CigoPage
{
    ...
    {
        config('cigo.VAR_PAGE') && $this->p = config('cigo.VAR_PAGE'); //设置分页参数名称
        /* 基础设置 */
        $this->totalRows = $totalRows; //设置总记录数
        $this->listRows = $listRows;  //设置每页显示行数
        $this->parameter = empty($parameter) ? Request::route() : $parameter;
        $this->nowPage = empty(Request::route($this->p)) ? 1 : intval(Request::route($this->p));
