<?php
namespace app\cigoadmin\library\cutdown;
...
public class SfReduceRule implements ReduceRule {
    ...
    public function getReduceList($totalReduce, $totalReduceTimes) {
        if (totalReduceTimes <= 0) {
            throw new Exception("total reduce times should gte 1");
        }

        // 最低砍价金额默认为： 总砍价金额 / (砍价次数 * 2)
        //TODO
//        $minReduce = $totalReduce.divide(BigDecimal.valueOf(totalReduceTimes).multiply(BigDecimal.valueOf(2)), BigDecimal.ROUND_HALF_EVEN);
        return getReduceList($totalReduce, $totalReduceTimes, $minReduce);
    }

    public function getReduceList($totalReduce, $totalReduceTimes, $minReduce) {
        $ret = array();
        $rand = new Random();
