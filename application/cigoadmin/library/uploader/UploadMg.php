 ...
 * 负责文件的上传处理
 */
class UploadMg implements IResponse
{
    private $args = false;
    private $files = false;
    ...
    public function init($args = false, $files = false, $configs = false)
    {
        ...
        if (!$configs) {
            $this->configs = config('cigo.FILE_UPLOAD');
        }
    ...
    public function makeFileUploader()
    {
        if (!isset($this->args['fileType']) || empty($this->args['fileType'])) {
            $this->makeResponse(false, null, '参数错误！', ErrorCode::ARGS_WRONG);
            return false;
        }
        $fileUpCls = 'app\\cigoadmin\\library\\uploader\\file\\' . ucfirst($this->args['fileType']);
        $this->fileUploader = new $fileUpCls();
        if (!$this->fileUploader) {
            $fileUpCls = 'app\\cigoadmin\\library\\uploader\\file\\File';
            $this->fileUploader = new $fileUpCls();
        }
        //初始化文件上传类
    ...
    private function checkUploadFile(&$file)
    {
        // 检查是否存在上传错误
        if ($file['error']) {
            $this->makeResponse(false, null, $this->checkUploadErrorNo($file['error']), ErrorCode::UPLOAD_TMP_FILE_ERROR);
            return false;
        }
        // 无效上传
        if (empty($file['name'])) {
            $this->makeResponse(false, null, '上传文件名为空！', ErrorCode::UPLOAD_TMP_FILE_ERROR);
            return false;
        }
        // 检查上传临时文件是否合法
        if (!is_uploaded_file($file['tmp_name'])) {
            $this->makeResponse(false, null, '非法上传文件！', ErrorCode::UPLOAD_TMP_FILE_ERROR);
            return false;
        }
