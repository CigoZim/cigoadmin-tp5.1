...
namespace app\cigoadmin\library\uploader;
 ...
 * 负责铁定文件的上传处理
 */
abstract class Uploader implements IResponse
{
    ...
    public function checkConfigs($file, $configs)
    {
        ...
        if (
            ...
            $this->makeResponse(false, null, '文件大小超出限制(' . ($this->maxSize / 1024 / 1024) . 'M)!', ErrorCode::UPLOAD_FILE_TOOBIG);
            return false;
        }
        //2. 检查文件后缀
        if (!$this->exts || !count($this->exts) || !in_array(strtolower($file['ext']), $this->exts)) {
            $this->makeResponse(false, null, '上传文件后缀不允许！', ErrorCode::UPLOAD_FILE_EXT_ERROR);
            return false;
        }
    ...
    private function saveUploadFileToDisk($dbInfo, &$file, $configs)
    {
        ...
        if (!move_uploaded_file($file['tmp_name'], $file['saved_path_name'])) {
            $this->makeResponse(false, null, '上传文件保存错误！', ErrorCode::UPLOAD_FILE_SAVE_ERROR);
            return false;
        }
        return true;
    }
    ...
    protected function mkPath($path)
    {
        if (is_dir($path)) {
            if (!is_writable($path)) {
                $this->makeResponse(false, null, '上传目录不可写！', ErrorCode::UPLOAD_PATH_NO_WRITABLE);
                return false;
            }
            return true;
        }
        if (!mkdir($path, 0777, true)) {
            $this->makeResponse(false, null, '目录 {' . $path . '} 创建失败！！', ErrorCode::UPLOAD_PATH_MKDIR_ERROR);
            return false;
        }
    ...
    protected function checkPathWritable($path)
    {
        /* 检测目录是否可写 */
        if (!is_writable($path)) {
            $this->makeResponse(false, null, '上传目录{' . $path . '}不可写！', ErrorCode::UPLOAD_PATH_NO_WRITABLE);
            return false;
        }
        return true;
    }
