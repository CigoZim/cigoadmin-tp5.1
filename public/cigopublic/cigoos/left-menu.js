;(function ($) {
	let leftMenu = function (ele, opt) {
		...
		instance.requestMenuData = function () {
            let target = $(instance.options.menu_container).attr('href') || $(instance.options.menu_container).attr('url');
            if (target !== undefined && target !== '' && target !== '#') {
				$.post(target, {}, function (data) {
					if (data.code === 1) {
						...
						let ulContent = new Array();
						instance.loadSubMenu(ulContent, data.data, 0);
		...
		instance.bindMenuItemClickEvent = function () {
			let target = $(instance.options.menu_container).attr('url-add-opt-rate');
...
//定义插件
	$.fn.leftMenu = function (options) {
		let menuInstance = new leftMenu(this, options);
		//进行初始化操作
		menuInstance.init();
